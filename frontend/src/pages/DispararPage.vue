      ...
      <div class="col-12 col-md-10 col-lg-8">
        ...
        <div class="q-mb-lg">
          <div class="row items-center justify-between q-gutter-sm">
            <div class="col">
              <!-- <div class="text-overline text-accent text-weight-medium q-mb-xs">
                IANO WhatsApp · Campanhas
              </div> -->
            ...
            <!-- <div class="col-auto" v-if="!isMobile">
              <q-chip outline color="accent" text-color="accent" icon="cloud_done">
                API: {{ apiBaseUrl }}
              </q-chip>
            </div> -->
          </div>
        ...
        <q-card class="q-pa-md q-pa-lg-md rounded-borders">
          <div class="col-auto q-mb-md">
            <q-btn-toggle v-model="contactMode" dense rounded unelevated color="grey" text-color="white"
              toggle-color="accent" toggle-text-color="" class="bg-grey-10" :options="[
...
(11) 99999-9999">
          ...
          <div class="q-mb-md">
            ...
            <!-- <div class="text-caption text-grey-5 q-mb-sm">
              Escolha o tipo de mensagem que será enviada para os contatos selecionados.
            </div> -->
          ...
          <div class="row items-center justify-between q-gutter-sm">
            ...
            <div class="col-12 col-md-auto text-right q-gutter-sm q-mt-sm q-mt-md-none">
              ...
              <q-btn unelevated color="accent" :disable="!canSendBulk || bulkLoading" :loading="bulkLoading"
...
function onFilterGroups(val, update) {
  update(() => {
    ...
    void needle; // reservado se quiser filtrar server-side depois
  });
}
...
const onSubmitBulk = async () => {
  ...
  if (contactMode.value === 'manual' && invalidContacts.value.length) {
    $q.notify({
      type: 'warning',
      position: 'top',
      message: 'Existem números inválidos na lista manual. Corrija antes de enviar.'
    });
    return;
  }
  ...
  try {
    ...
    $q.notify({
      ...
      position: 'top',
      icon: 'error',
      message: err?.response?.data?.error || err?.message || 'Erro ao enviar campanha.'
    });
  } finally {
...
async function onSaveGroup() {
  ...
  try {
    ...
    $q.notify({
      ...
      position: 'top',
      icon: 'check_circle',
      message: 'Grupo salvo com sucesso! Ele já está disponível na aba "Grupo salvo".'
    });
    ...
    $q.notify({
      ...
      position: 'top',
      icon: 'error',
      message: err?.response?.data?.error || err?.message || 'Erro ao salvar grupo.'
    });
  } finally {
...
.save-group-list {
  ...
  padding: 4px 10px;
  background: radial-gradient(circle at top left, rgba(30, 64, 175, 0.25), transparent 60%);
}
</style>
